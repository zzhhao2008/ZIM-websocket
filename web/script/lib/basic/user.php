<?php
$GLOBALS['username'] = $_COOKIE['login_name_'.$comname];
...
class user
{
    ...
    public static function LoginCheck($init = 0)
    {
        ...
        global $comname;
        if ($_COOKIE['login_pas_'.$comname] === md5($thisup['llt'] . $thisup['password'])) {
    ...
    public static function login($name, $pas)
    {
        ...
        if ($cfg['password'] === md5($pas)) {
            ...
            setcookie("login_name_$comname", $name, time() + 3600 * 48, "/");
            setcookie("login_pas_$comname", $pass, time() + 3600 * 48);
